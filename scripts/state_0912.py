"""State read for the 2026-09-12 02:05 overnight run. Read-only.

Same shape as state_0910.py — queue A's SupplierPart pool closed 2026-09-01 and
queue B on 09-06, so what matters is the DELTA (parts created since closure,
imageless parts on genuinely OPEN POs), queue D's null-safe keyword backlog, and
queue C's sweep window.
"""
import os
import re
import sys
...
print()
print("-- parts created in the last 6 days --")
...
for po in PurchaseOrder.objects.order_by("-pk")[:20]:
...
try:
    with open(pend) as fh:
        for ln in fh:
            if ln.strip().startswith("- [ ]") or ln.strip().startswith("- [x] APPROVED"):
                print(f"   {ln.rstrip()[:170]}")
except OSError as exc:
